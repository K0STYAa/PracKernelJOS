...
boot_alloc(uint32_t n) {
  ...
  if (!nextfree) {
    extern char end[];
    nextfree = ROUNDUP((char *)end, PGSIZE); // берем первый адрес кратный размеру страницы(not kernel code и не под глобальные переменные)
  }
  ...
  nextfree += ROUNDUP(n, PGSIZE); // берем нужное число байтов
  
  if (PADDR(nextfree) > PGSIZE * npages) {
      panic("Out of memory on boot_alloc");
  }
...
mem_init(void) {
  ...
  pml4e = boot_alloc(PGSIZE); // берем страничку
  memset(pml4e, 0, PGSIZE); // заполняем нулями
  kern_pml4e = pml4e; 
  ...
  pages = (struct PageInfo *) boot_alloc(sizeof(struct PageInfo) * npages); // выделяем
  memset(pages, 0, sizeof(struct PageInfo) * npages); // заполняем 

  //////////////////////////////////////////////////////////////////////
  // Make 'envs' point to an array of size 'NENV' of 'struct Env'.
  // LAB 8: Your code here.
  envs = (struct Env *) boot_alloc(sizeof(struct Env) * NENV);
  memset(envs, 0, sizeof(struct Env) * npages);
  //////////////////////////////////////////////////////////////////////
