...
sys_env_set_pgfault_upcall(envid_t envid, void *func) {
  ...
  // TODO: Не забудьте проверку разрешений при поиске идентификаторов процессов, так как это «опасный» системный вызов.
...
sys_page_alloc(envid_t envid, void *va, int perm) {
  ...
  struct PageInfo *pi;
  if ((pi = page_alloc(ALLOC_ZERO)) < 0) {
    return -E_NO_MEM;
  }

  if (page_insert(e->env_pml4e, pi, va, perm | PTE_U) < 0) {
    page_free(pi);
    return -E_NO_MEM;
  }
...
sys_page_map(envid_t srcenvid, void *srcva,
  ...
  // LAB 9: Your code here.
  struct Env *envsrc = NULL;
  struct Env *envdst = NULL;

  struct PageInfo *pi;

  pte_t *pte;

  envid2env(srcenvid, &envsrc, 1);
  envid2env(dstenvid, &envdst, 1);
  if (envsrc == NULL || envdst == NULL) {
    return -E_BAD_ENV;
  }

  if ((uint64_t)srcva >= UTOP || PGOFF(srcva) || (uint64_t)dstva >= UTOP || PGOFF(dstva)) {
    return -E_INVAL;
  }
  ...
  if ((pi = page_lookup(envsrc->env_pml4e, srcva, &pte)) == NULL) {
    return -E_INVAL;
  }
